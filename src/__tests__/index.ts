...
fs.readdirSync(FIXTURES).forEach(fixture => {
  test(fixture, async () => {
    ...
    generated.output.forEach((output) => {
      ...
      const source = ((output as any).source || (output as any).code).toString("utf-8").replace(/@marko\/rollup\$\d\.\d\.\d/g, "@marko/rollup$latest");
      expect(source).toMatchFile(
        path.join(snapshotDir, output.fileName)
      );
      console.log
      // expect(code).toM
    });
